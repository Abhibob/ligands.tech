...
def _batch_workflow_instructions() -> str:
    ...
    return (
        ...
        "## Step 2: Batch dock/predict with --ligand-dir and --top-n\n"
        "gnina REQUIRES: (1) PDB format for --receptor (NEVER CIF), and (2) a search space "
        "via --autobox-ligand with one of the downloaded SDF files from the ligands directory. "
        "Use list_files on the ligands directory to find a file name for --autobox-ligand.\n"
        "```\n"
        "bind-gnina dock --receptor <protein_pdb_path> --ligand-dir ligands/ "
        "--autobox-ligand ligands/<any_ligand>.sdf --top-n 20 "
        "--artifacts-dir docking/ --json-out results/gnina-batch.json\n"
        "```\n"
        ...
        "read_file -> results/gnina-batch.json -> extract `artifacts.manifestPath`\n"
        "read_file -> the manifest path to see ranked docking results\n\n"
        "```\n"
        "bind-boltz predict --protein-fasta <fasta_path> --ligand-dir ligands/ --top-n 20 "
        "--artifacts-dir boltz/ --json-out results/boltz-batch.json\n"
        "```\n"
        "read_file -> results/boltz-batch.json -> extract `artifacts.manifestPath`\n"
        "read_file -> the manifest path to see ranked prediction results\n\n"

        "## Step 3: Batch validate with --pred-dir / --complex-dir\n"
        "```\n"
        "bind-posebusters check --pred-dir boltz/ --top-n 10 --json-out results/posebusters-batch.json\n"
        "```\n"
        "read_file -> results/posebusters-batch.json -> extract `artifacts.manifestPath`\n"
        "read_file -> the manifest path to see validation results\n\n"
        "```\n"
        "bind-plip profile --complex-dir boltz/ --top-n 10 --json-out results/plip-batch.json\n"
        "```\n"
        "read_file -> results/plip-batch.json -> extract `artifacts.manifestPath`\n"
        "read_file -> the manifest path to see interaction profiles\n\n"
        ...
        "## Batch + subagents\n"
        "For very large screens (>100 compounds), split work across subagents:\n"
        "- Main agent resolves protein + downloads binders\n"
        "- Spawn subagent A: gnina dock --receptor X --ligand-dir ligands/ "
        "--autobox-ligand ligands/<first>.sdf --top-n 20\n"
        "- Spawn subagent B: boltz predict --protein-fasta X --ligand-dir ligands/ --top-n 20\n"
        "- Collect results, then run posebusters + plip on top hits\n\n"
